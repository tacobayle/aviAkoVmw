...
resource "vsphere_virtual_machine" "jump" {
  ...
  provisioner "remote-exec" {
    inline      = [
      ...
      "cd ~/ansible ; git clone ${var.ansible["k8sInstallUrl"]} --branch ${var.ansible["k8sInstallTag"]} ; ansible-playbook -i /opt/ansible/inventory/inventory.vmware.yml ansibleK8sInstall/main.yml",
    ]
  }
